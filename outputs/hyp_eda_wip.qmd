...
The main point to note from this chart is that GP hypertension with a record of treated to threshold reached the highest percentage up to 81.6% of people 80 and over were recorded over this period.
...
```{r}
#| fig-height: 9
#| fig-cap: Timeseries of high risk – one high blood pressure for people with no hypertension in England by the intersection of age group and gender
...
df <-
  ...
  dplyr::mutate(
    ...
    text = glue::glue(
      "<b>{time_period_name_f}</b>\n",
      "{series} <b>({metric_category_name})</b>\n",
      "{round(value, digits = 1)}% high risk-high BP\n",
      "Numerator: {prettyunits::pretty_num(numerator)}\n",
      "Denominator: {prettyunits::pretty_num(denominator)}"
    )
  ) |> 
...
```{r}
#| fig-height: 9
#| fig-cap: Timeseries of potential antihypertensive overtreatment for people with hypertension in England by the intersection of age group and gender
...
```{r}
#| fig-height: 9
#| fig-cap: Timeseries of treated to appropriate threshold for people with hypertension in England by the intersection of age group and gender
...
```{r}
#| fig-height: 8
#| fig-cap: Timeseries of hypertension monitoring with ACR in England
...
df <-
  ...
  # limit to metric and overall measure
  dplyr::filter(
    indicator_code == 'CVDP009HYP',
    metric_category_name == 'Persons', # get the overall value
  ) |> 
  dplyr::mutate(
    # how should the series be defined?
    series = "Monitoring with ACR" |> forcats::fct(),
    # create hovertext
    text = glue::glue(
      "<b>{time_period_name_f}</b>\n",
      "{round(value, digits = 1)}% monitoring with ACR\n",
      "Numerator: {prettyunits::pretty_num(numerator)}\n",
      "Denominator: {prettyunits::pretty_num(denominator)}"
    )
  ) |> 
...
df |> 
  plot_timeseries(
    plot_title = "England hypertension monitoring with ACR (CVDP009HYP)",
    y_axis_title = "People with hypertension monitoring ACR\n(percent)"
  )
```
...
```{r}
#| fig-height: 8
#| fig-cap: Timeseries of the rate of people hypertension monitoring with ACR in England by gender
...
df |> 
  plot_timeseries(
    plot_title = "England hypertension monitoring with ACR (CVDP009HYP) - gender",
    y_axis_title = "People hypertension monitoring with ACR\n(percent)"
  )
```
...
```{r}
#| fig-height: 8
#| fig-cap: Timeseries of the rate of people hypertension monitoring with ACR in England by age group
...
df |> 
  plot_timeseries(
    plot_title = "England hypertension monitoring with ACR (CVDP009HYP) - age group",
    y_axis_title = "hypertension monitoring with ACR\n(percent)"
  )
```
...
```{r}
#| fig-height: 8
#| fig-cap: Timeseries of the rate of people hypertension monitoring with ACR in England by ethnicity
...
df <-
  ...
  dplyr::mutate(
    ...
    text = glue::glue(
      "<b>{time_period_name_f}</b>\n",
      "{series}\n",
      "{round(value, digits = 1)}% hypertension monitoring with ACR\n",
      "Numerator: {prettyunits::pretty_num(numerator)}\n",
      "Denominator: {prettyunits::pretty_num(denominator)}"
    )
  ) |> 
...
df |> 
  plot_timeseries(
    plot_title = "England hypertension monitoring with ACR (CVDP009HYP) - broad ethnic group",
    y_axis_title = "People hypertension monitoring with ACR\n(percent)"
  )
```
...
```{r}
#| fig-height: 8
#| fig-cap: Timeseries of the rate of people hypertension monitoring with ACR in England by deprivation quintile
...
df |> 
  plot_timeseries(
    plot_title = "England hypertension monitoring with ACR (CVDP008HYP) - deprivation",
    y_axis_title = "People hypertension monitoring with ACR \n(percent)"
  )
```
...
```{r}
#| fig-height: 8
#| fig-cap: Timeseries of the rate of people hypertension monitoring with ACR in England by mental health status
...
df |> 
  plot_timeseries(
    plot_title = "England hypertension monitoring with ACR (CVDP009HYP) - SMI",
    y_axis_title = "People hypertension monitoring with ACR\n(percent)"
  )
```
...
```{r}
#| fig-height: 8
#| fig-cap: Timeseries of the rate of people hypertension monitoring with ACR in England by mental learning disability status
...
df |> 
  plot_timeseries(
    plot_title = "England hypertension monitoring with ACR (CVDP009HYP) - LD",
    y_axis_title = "People hypertension monitoring with ACR\n(percent)"
  )
```
...
```{r}
#| fig-height: 9
#| fig-cap: Timeseries of monitoring with ACR for people with hypertension in England by the intersection of age group and gender
...
Hypertension monitoring with ACR rates are distributed unevenly across ICBs, with some having lower rates and others higher rates in each reporting period.

*Box-plots* provide one way to summarise distributions that allow comparisons across time, for example.

```{r}
#| fig-height: 9
#| fig-cap: Distributions of rates of people hypertension monitoring with ACR per ICBs over successive reporting periods
...
df_metrics_icb |> 
  ...
  plotly::layout(
    title = "ICB distribution -  hypertension monitoring with ACR (CVDP009HYP)",
    font = plotly_font_family,
    xaxis = list(
      title = "People hypertension monitoring with ACR (percent)", 
      rangemode = "tozero"
    ),
    yaxis = list(title = "")
  ) |> 
...
df_dep_icb |> 
  ...
  plotly::layout(
    title = "ICB distribution - hypertension monitoring with ACR (CVDP009HYP)",
    font = plotly_font_family,
    xaxis = list(title = "hypertension monitoring with ACR (percent)")
  ) |> 
  plotly::animation_slider(
    currentvalue = list(prefix = "", font = list(color = "black"))  
  ) |> 
...
df |> 
  ...
  plotly::layout(
    ...
    xaxis = list(title = ""),
    yaxis = list(
      title = "People hypertension monitoring with ACR\n(percent)",
      rangemode = "tozero"
    )
  ) |> 
  configure_plotly()
...
```{r}
#| fig-height: 6
#| fig-cap: Trend in inequality (SII) of crude rates of hypertension monitoring with ACR
...
if (params$flag_calc_expensive_operations) {
  ...
  saveRDS(
    object = df_dep_icb_sii,
    file = here::here('data', 'project', 'df_dep_icb_sii_p009.Rds')
  )
}
```
...
```{r}
#| fig-height: 7
#| fig-cap: Geographical distribution of SII using crude rates of people hypertension monitoring with ACR
